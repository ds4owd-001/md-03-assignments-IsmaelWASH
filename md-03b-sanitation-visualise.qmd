...
*We have 8 variables in the data set*
...
```{r}
sanitation_niger <- sanitation |> filter(name=="Niger",year==2000|year==2020,varname_short!="san_sm")
```
...
```{r}
ggplot(data=sanitation_niger,mapping = aes(x=residence,y=percent,fill=varname_long))+
  geom_col()+
  facet_wrap(~year)+
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend?
    ...
    *The order is reversed. In practice, the basic service level is at the bottom of the bar.*

2.  What would you want to change?

    *The R code needs to be revised to present the graphic as it is presented by the JMP (Basic at the bottom, then limited, no facilities and finally all the way to the top, unimproved).*

3.  Why did we remove "safely managed sanitation services" from the data set in Task 3?

    *We have removed the safely managed sanitation services, in order to have a 100% distribution between the different service levels. With safely managed sanitation services, we will exceed the 100% mark, which is not correct.*
...
2.  What do you observe when you look at the code and plot?
    ...
    *If you look at the graph, you will see several charts showing histograms based on place of residence and service levels.*
