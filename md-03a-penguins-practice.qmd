...
*In this data set we have 8 variables and 344 observations.*

*Data types for each variable:*

-   *species: categorial variable*

-   *island: categorial variable*

-   *bill_length_mm: continuous variable*

-   *bill_depth_mm: continuous variable*

-   *flipper_length_mm: discrete variable*

-   *body_mass_g: discrete variable*
...
```{r}
penguins |> filter(body_mass_g<=2900)
```
...
```{r}
penguins |> filter(species!="chinstraps")
```
...
```{r}
penguins |> filter(species=="Gentoo",bill_depth_mm>=15.5)
```
...
```{r}
penguins |> filter(sex=="male",island=="Dream"|island=="Biscoe")
```
...
```{r}
penguins |>  filter(sex=="female",bill_length_mm<35)
```
...
```{r}
penguins |> filter(sex=="female",species=="Chinstrap",island=="Dream"|island=="Torgersen")
```
...
```{r}
penguins |> filter(xor(species=="Gentoo",body_mass_g>4500))
```
...
```{r}
penguins <-  mutate(penguins,flipper_m=flipper_length_mm/1000)
```
...
```{r}
penguins <- mutate(penguins,id=seq(1:n())) |> relocate(id)
```
...
```{r}
penguins |> filter(!is.na(flipper_length_mm)) |> group_by(species) |> summarise(flip_max=max(flipper_length_mm),flip_min=min(flipper_length_mm))
```

*To avoid NA, from the outset we filter the data by removing all NA observations.*
...
```{r}
penguins |> filter(!is.na(bill_depth_mm)) |> group_by(species,year) |> summarise(bill_depth_mean=mean(bill_depth_mm),bill_length_mean=mean(bill_length_mm))
```
...
```{r}
penguins |> filter(sex=="female",island=="Torgersen"|island=="Biscoe") |> count(sex,island)
```
